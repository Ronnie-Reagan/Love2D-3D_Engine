...
function M.create(cubeModel, q)
	...
	return {
		...
		terrainSdf = {
			...
			meshBuildBudget = 4,
			workerMaxInflight = 6,
			workerResultBudgetPerFrame = 4,
			workerResultTimeBudgetMs = 3.0,
			maxAdaptiveLod1Radius = 8,
			maxPendingChunks = 96,
			maxStaleChunks = 32,
			maxDisplayedChunks = 512,
			maxDisplayedChunksHardCap = 8192,
			drawDistanceOverridesLodRadius = true,
			splitLodEnabled = false,
			highResSplitRatio = 0.5,
			autoQualityEnabled = false,
			targetFrameMs = 16.6,
			chunkCacheLimit = 128,
			farLodConeEnabled = true,
			farLodConeDegrees = 110,
			rearLod2Radius = 4,
			minY = -120,
			maxY = 1200,
			baseHeight = 0,
