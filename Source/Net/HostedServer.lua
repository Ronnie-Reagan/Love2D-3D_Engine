...
function hostedServer.create(opts)
	...
	function server:addCrater(crater)
		...
		local next = terrainSdfSystem.normalizeGroundParams(params, defaultGroundParams)
		next.dynamicCraters = next.dynamicCraters or {}
		next.dynamicCraters[#next.dynamicCraters + 1] = {
		...
		self.world.groundParams = next
		self:broadcast(buildCraterPacket(next.dynamicCraters[#next.dynamicCraters]), 0)
	end

	function server:setGroundParams(params)
		self.world.groundParams = terrainSdfSystem.normalizeGroundParams(params, defaultGroundParams)
		self:broadcast(buildWorldSyncPacket(self.world), 0)
	end
	...
	function server:serviceClientEvent(event)
		...
		elseif pType == "VOICE_FRAME" then
			for _, other in pairs(self.players) do
				if other.peer and other.id ~= peerId and other.radioChannel == player.radioChannel then
